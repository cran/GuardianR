...
get_json.nokey <- function(keywords, format="json", from.date, to.date)
{
  ...
  {
    request <- paste("http://content.guardianapis.com/search?q=", keywords, "&from-date=", from.date, "&to-date=", to.date, "&format=", format, "&show-fields=all&page=", this.page, "&pageSize=", page.size, sep="")
...
parse_json_to_df.nokey <- function(api.responses.nokey)
{
  ...
  pages <- try(seq(from=9, to=length(api.responses.nokey), by=9), silent=T)
  
  for (i in pages)
  {
    for (j in 1:length(api.responses.nokey[i]$results))
    {
      id <- api.responses.nokey[i]$results[[j]]$id
      sectionId <- api.responses.nokey[i]$results[[j]]$sectionId
      sectionName <- api.responses.nokey[i]$results[[j]]$sectionName
      webPublicationDate <- api.responses.nokey[i]$results[[j]]$webPublicationDate
      webTitle <- api.responses.nokey[i]$results[[j]]$webTitle
      webUrl <- api.responses.nokey[i]$results[[j]]$webUrl
      apiUrl <- api.responses.nokey[i]$results[[j]]$apiUrl
      newspaperPageNumber <- api.responses.nokey[i]$results[[j]]$fields$newspaperPageNumber
      trailText <- api.responses.nokey[i]$results[[j]]$fields$trailText
      headline <- api.responses.nokey[i]$results[[j]]$fields$headline
      showInRelatedContent <- api.responses.nokey[i]$results[[j]]$fields$showInRelatedContent
      lastModified <- api.responses.nokey[i]$results[[j]]$fields$lastModified
      hasStoryPackage <- api.responses.nokey[i]$results[[j]]$fields$hasStoryPackage
      score <- api.responses.nokey[i]$results[[j]]$fields$score
      standfirst <- api.responses.nokey[i]$results[[j]]$fields$standfirst
      shortUrl <- api.responses.nokey[i]$results[[j]]$fields$shortUrl
      wordcount <- api.responses.nokey[i]$results[[j]]$fields$wordcount
      commentable <- api.responses.nokey[i]$results[[j]]$fields$commentable
      allowUgc <- api.responses.nokey[i]$results[[j]]$fields$allowUgc
      isPremoderated <- api.responses.nokey[i]$results[[j]]$fields$isPremoderated
      byline <- api.responses.nokey[i]$results[[j]]$fields$byline
      publication <- api.responses.nokey[i]$results[[j]]$fields$publication
      newspaperEditionDate <- api.responses.nokey[i]$results[[j]]$fields$newspaperEditionDate
      shouldHideAdverts <- api.responses.nokey[i]$results[[j]]$fields$shouldHideAdverts
      liveBloggingNow <- api.responses.nokey[i]$results[[j]]$fields$liveBloggingNow
      commentCloseDate <- api.responses.nokey[i]$results[[j]]$fields$commentCloseDate
...
{
  guardian.api.responses.nokey <- get_json.nokey(keywords, format="json", from.date, to.date)
  guardian.api.df.nokey <- parse_json_to_df.nokey(guardian.api.responses.nokey)
  return (guardian.api.df.nokey)
}
